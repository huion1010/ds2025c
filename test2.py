...
class DisjointSet: # 크루스칼 알고리즘을 위한 클래스
    def __init__(self, n):
    ...
    def merge(self, x, y):
        ...
        if x_root != y_root:                # if 0 != 1:
            self.parent[y_root] = x_root    # self.parent[1] = 0
            return True
        return False
...
edge_array.sort(reverse=False) # 오름차순
print(edge_array)
...
for cost, s, e in edge_array:
    if ds.merge(s, e):
        mst_edges.append((cost, s, e)) # 최소 간선 추가
        mst_cost += cost # 최소 비용 업데이트
...
print("최소 간선 목록")
for cost, s, e in mst_edges:
    print(f"{city_array[s]} --- {city_array[e]} : {cost}")
